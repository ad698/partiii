...
params = {'backend': 'Cairo','axes.labelsize': 9,'text.fontsize': 9,'legend.fontsize': 9,
...
def plot_closed_short():
    f = plt.figure(figsize=[3.32,3.32])
    ...
    f.savefig('Delta1.pdf',bbox_inches='tight')
...
def plot_closed_long():
    ...
    f.savefig('Delta2.pdf',bbox_inches='tight')


def Delta_withEnergy():
    f = plt.figure(figsize=[3.32,3.32 * 0.7])
    ...
    f.savefig('Delta_withE.pdf',bbox_inches='tight')
...
def DeltaCheck():
    ...
    a[0].axhline(np.sqrt(-13.3317 + 20.1094),color='k',linestyle='--',linewidth=1,zorder=0)
    a[1].scatter(Delta.T[0],np.sqrt(-1. - Delta.T[3]),c=Delta.T[1],label=r'$\Delta_2/{\rm kpc}$',s=4.3,edgecolor="None",zorder=1)
    a[1].axhline(np.sqrt(-1 + 13.3317),linewidth=1,color='k',linestyle='--',zorder=1)
    a[0].set_xticks([])
    ...
    f.savefig('DeltaCheck.pdf',bbox_inches='tight')
...
def sig_clipped(p):
    std = np.std(p.T[17:20],axis=1)
    mean = np.mean(p.T[17:20],axis=1)
    q = np.array([[j for j in p.T[17+i] if (j>mean[i]-3.*std[i])*(j<mean[i]+3.*std[i])] for i in range(3)])
    return np.array([np.mean(q[i]) for i in range(3)])


def testLoop_angles(infile1,infile2,infile3,outFile):
    timestep=0.005
    f,a = plt.subplots(3,2,figsize=[5.,5. * 1.5])
    plt.subplots_adjust(wspace=0.2,hspace=0.4)
    MAXI=200
    box = np.genfromtxt(infile1)
    freqs = sig_clipped(box)
    box = box[:MAXI]
    sal = np.genfromtxt(infile2)[:MAXI]
    lal = np.genfromtxt(infile3)[:MAXI]
    for i in a:
        for j in i:
            j.set_xlim(0,1)
            j.set_ylim(0,1)
            j.set_aspect('equal')
    a[0,0].scatter(box.T[14] / 2. / np.pi,box.T[15] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)
    a[0,1].scatter(box.T[14] / 2. / np.pi,box.T[16] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)

    results = np.array([[(box[0][14 + j] + timestep * i * freqs[j]) % (2. * np.pi) / 2. / np.pi for j in range(3)] for i in range(MAXI)])
    counter1, counter2 = 0,0
    counter11, counter22 = 0,0
    for i in range(len(results) - 1):
        if ((results[i][0]>results[i + 1][0]) or (results[i][1]>results[i + 1][1])) and counter11<2:
            a[0,0].plot(results.T[0][counter1:i + 1],results.T[1][counter1:i + 1],c=colC,zorder=0)
            counter1=i + 1
            counter11+=1
        if ((results[i][0]>results[i + 1][0]) or (results[i][2]>results[i + 1][2])) and counter22<2:
            a[0,1].plot(results.T[0][counter2:i + 1],results.T[2][counter2:i + 1],c=colC,zorder=0)
            counter2=i + 1
            counter22+=1
    # a[0,0].plot(results.T[0][counter1:],results.T[1][counter1:],c=colC,zorder=0)
    # a[0,1].plot(results.T[0][counter2:],results.T[2][counter2:],c=colC,zorder=0)
    a[0,0].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[0,1].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[0,0].set_ylabel(r'$\theta_\mu/2\pi$')
    a[0,1].set_ylabel(r'$\theta_\nu/2\pi$')
    a[0,0].text(0, 1.02, 'Box', horizontalalignment='left', verticalalignment='bottom', transform=a[0,0].transAxes, fontsize=10)

    sal = np.genfromtxt(infile2)
    freqs = sig_clipped(sal)
    sal = sal[:MAXI]
    a[1,0].scatter(sal.T[14] / 2. / np.pi,sal.T[15] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)
    a[1,1].scatter(sal.T[14] / 2. / np.pi,sal.T[16] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)
    freqs = np.mean(sal.T[17:20],axis=1)
    results = np.array([[(sal[0][14 + j] + timestep * i * freqs[j]) % (2. * np.pi) / 2. / np.pi for j in range(3)] for i in range(MAXI)])
    counter1, counter2 = 0,0
    counter11, counter22 = 0,0
    for i in range(len(results) - 1):
        if ((results[i][0]>results[i + 1][0]) or (results[i][1]>results[i + 1][1])) and counter11<3:
            a[1,0].plot(results.T[0][counter1:i + 1],results.T[1][counter1:i + 1],c=colC,zorder=0)
            counter1=i + 1
            counter11+=1
        if ((results[i][0]>results[i + 1][0]) or (results[i][2]>results[i + 1][2])) and counter22<2:
            a[1,1].plot(results.T[0][counter2:i + 1],results.T[2][counter2:i + 1],c=colC,zorder=0)
            counter2=i + 1
            counter22+=1
    # a[1,0].plot(results.T[0][counter1:],results.T[1][counter1:],c=colC,zorder=0)
    # a[1,1].plot(results.T[0][counter2:],results.T[2][counter2:],c=colC,zorder=0)
    a[1,0].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[1,1].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[1,0].set_ylabel(r'$\theta_\mu/2\pi$')
    a[1,1].set_ylabel(r'$\theta_\nu/2\pi$')
    a[1,0].text(0, 1.02, 'Short-axis loop', horizontalalignment='left', verticalalignment='bottom', transform=a[1,0].transAxes, fontsize=10)

    lal = np.genfromtxt(infile3)
    freqs = sig_clipped(lal)
    print(freqs)
    lal = lal[:MAXI]
    a[2,0].scatter(lal.T[14] / 2. / np.pi,lal.T[15] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)
    a[2,1].scatter(lal.T[14] / 2. / np.pi,lal.T[16] / 2. / np.pi,c='k',edgecolor="None",s=2,zorder=1)
    freqs = np.mean(lal.T[17:20],axis=1)
    results = np.array([[(lal[0][14 + j] + timestep * i * freqs[j]) % (2. * np.pi) / 2. / np.pi for j in range(3)] for i in range(MAXI)])
    counter1, counter2 = 0,0
    counter11, counter22 = 0,0
    for i in range(len(results) - 1):
        if ((results[i][0]>results[i + 1][0]) or (results[i][1]>results[i + 1][1])) and counter11<3:
            a[2,0].plot(results.T[0][counter1:i + 1],results.T[1][counter1:i + 1],c=colC,zorder=0)
            counter1=i + 1
            counter11+=1
        if ((results[i][0]>results[i + 1][0]) or (results[i][2]>results[i + 1][2])) and counter22<3:
            a[2,1].plot(results.T[0][counter2:i + 1],results.T[2][counter2:i + 1],c=colC,zorder=0)
            counter2=i + 1
            counter22+=1
    # a[2,0].plot(results.T[0][counter1:],results.T[1][counter1:],c=colC,zorder=0)
    # a[2,1].plot(results.T[0][counter2:],results.T[2][counter2:],c=colC,zorder=0)
    a[2,0].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[2,1].set_xlabel(r'$\theta_\lambda/2\pi$')
    a[2,0].set_ylabel(r'$\theta_\mu/2\pi$')
    a[2,1].set_ylabel(r'$\theta_\nu/2\pi$')
    a[2,0].text(0, 1.02, 'Long-axis loop', horizontalalignment='left', verticalalignment='bottom', transform=a[2,0].transAxes, fontsize=10)

    plt.savefig(outFile,bbox_inches='tight')


def plot_SoS(infile1, infile2, outFile, axis, axis2, text):
    f = plt.figure(figsize=[2.21,2.21 * 0.7])
    c = colors.ColorConverter().to_rgb
    rvb = make_colormap([c("#91bfdb"), c("#ffffbf"), 1. / 2., c("#ffffbf"),c("#fc8d59")])
...
def twoDdensity(infile,infile2,text,text2,outfile):
    data = np.genfromtxt(infile)
    f,a = plt.subplots(1,3,figsize=[9.6,2.6])
    ...
    CC = a[0].contour(X,Y,Z,levels=[0.5,1.,1.5,2.,2.5,3.,3.5,4.,4.5,5.,5.5,6.])
    print(CC.levels)
    ...
    CC = a[1].contour(X,Y,Z,levels=[0.5,1.,1.5,2.,2.5,3.,3.5,4.,4.5,5.,5.5,6.])
    print(CC.levels)
...
def error_planes():
    ...
    ccc.set_label(r'${\bf Orbit\,class}$',fontsize=11)
    ccc.ax.xaxis.set_ticks_position('top')
    ccc.ax.xaxis.set_label_position('top')
...
def moment_errors_with_actions():
    data = np.genfromtxt("density_noise.dat")
    f,a = plt.subplots(2,1,figsize=[5,7.5])
    x = data.T[0][1:] - data[0][0]
    a[0].set_xscale('log')
    a[0].set_xlim(np.power(10.,-1.5),np.power(10.,2.5))
    a[0].set_ylim(0.0000001,1.)
    a[0].set_yscale('log')
    a[0].set_xlabel(r'$\sqrt{\langle (\Delta J)^2 \rangle}/{\rm\, kpc\,km\,s}^{-1}$')
    a[0].set_ylabel(r'$|\Delta\rho|/\rho$')
    X = np.logspace(-1.5,2.5)
    labels = [r'$(x,y,z)=(4.0,0.5,0.5){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(0.5,4.0,0.5){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(0.5,0.5,4.0){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(4.0,4.0,4.0){\rm kpc},\,K = %.2f \pm %.2f$']
    colors = ["#e41a1c","#377eb8","#4daf4a","#984ea3"]
    for i in range(len(data.T) - 1):
        y = np.abs(data.T[i + 1][1:] - data[0][i + 1]) / data[0][i + 1]
        res = linregress(np.log(x),np.log(y))
        a[0].plot(x,y,color=colors[i],linewidth=1,marker='o',label= labels[i] % (res[0],res[4]))
        a[0].plot(X,np.power(X,res[0]) * np.exp(res[1]),color='gray',linewidth=1,alpha=0.6,linestyle='--')
    a[0].legend(loc=4,fontsize=9)

    data = np.genfromtxt("density_alpha3.dat")
    a[1].set_xscale('log')
    a[1].set_xlim(np.power(10.,-1.5),np.power(10.,1.5))
    a[1].set_ylim(0.000001,1.)
    a[1].set_yscale('log')
    a[1].set_xlabel(r'$\sqrt{\langle (\Delta J)^2 \rangle}/{\rm\, kpc\,km\,s}^{-1}$')
    a[1].set_ylabel(r'$|\Delta\rho|/\rho$')
    X = np.logspace(-1.5,1.5)
    labels = [r'$(x,y,z)=(4.0,0.5,0.5){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(0.5,4.0,0.5){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(0.5,0.5,4.0){\rm kpc},\,K = %.2f \pm %.2f$',
              r'$(x,y,z)=(4.0,4.0,4.0){\rm kpc},\,K = %.2f \pm %.2f$']
    for i in range(4):
        # x = np.sqrt(np.abs(data.T[i * 5 + 2][1:] ** 2 + data.T[i * 5 + 3][1:] ** 2 + data.T[i * 5 + 4][1:] ** 2))
        # x = data.T[i * 5 + 2][1:]
        x = data.T[i * 5 + 5][1:]
        y = np.abs(data.T[i * 5 + 1][1:] - data[0][i + 1]) / data[0][i + 1]
        res = linregress(np.log(x),np.log(y))
        a[1].plot(x,y,color=colors[i],linewidth=1,marker='o',label= labels[i] % (res[0],res[4]))
        a[1].plot(X,np.power(X,res[0]) * np.exp(res[1]),color='gray',linewidth=1,alpha=0.6,linestyle='--')
    a[1].legend(loc=4,fontsize=9)
    plt.savefig("moments_with_errors.eps",bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    # testLoop_angles("testloop_v2_with_angles.dat","testloop_v1_with_angles.dat","testloop_v3_with_angles.dat","test_angles.eps")
